...
class DCIPMModelling(pg.Modelling):
...
class ERTTLmod(pg.Modelling):  # outdated => use FullFrameModelling
    ...
    def __init__(self, nf=0, data=None, mesh=None, fop=None, rotate=False,
        ...
        for i in range(nf):
            if type(data) is list:
                fopi = ert.ERTModelling(data=data[i])
            else:
                fopi = ert.ERTModelling(data=data)
        ...
        self.mesh3D = pg.meshtools.createMesh3D(self.pd2d, z, 0, 0)
        if rotate:
...
class ERTMultiPhimod(pg.Modelling):  # use FullFrameModelling
    ...
    def __init__(self, pd, J2d, nf, rotate=False, verbose=False):
        ...
        self.nd = J2d.rows()
        self.J2d = J2d
        self.FOP2d = pg.frameworks.LinearModelling(J2d)
        self.FOP2d.setMesh(pd)
        self.J = pg.matrix.BlockMatrix()
        for i in range(self.nf):
